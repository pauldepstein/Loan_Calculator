...
const validateInterestRateHandling = require('../../exceptions/parameterValidators');
const ValidationErrorHandling = require('../../exceptions/validationError');

let validateInterest = validateInterestRateHandling.validateInterestRate;
...
try {
  ...
  if (error instanceof ValidationErrorHandling.ValidationError) {
    console.log(`Caught expected error: ${error.message}`);
  } else {
    console.log(`Unexpected error: ${error}`);
  }
